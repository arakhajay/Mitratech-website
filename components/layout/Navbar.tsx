              ...
              if (link.hasDropdown) {
                return (
                  ...
                  >
                    ...
                    {/* Mega Menu Dropdown */}
                    {isServicesOpen && (
                      <div className="absolute top-full left-1/2 -translate-x-1/2 w-[700px] pt-3 animate-fadeIn">
                        <div className="p-6 rounded-2xl glass-panel border border-slate-700/80 shadow-2xl grid grid-cols-2 gap-4">
                          {SERVICES_DATA.map((service) => (
                            <Link
                              key={service.id}
                              href={`/services/${service.slug}`}
                              onClick={() => setIsServicesOpen(false)}
                              className="flex items-start space-x-3.5 p-3 rounded-xl hover:bg-slate-800/80 border border-transparent hover:border-slate-700 transition-all group"
                            >
                              <div className="p-2.5 rounded-lg bg-slate-900 border border-slate-800 group-hover:scale-110 transition-transform">
                                {iconMap[service.iconName]}
                              </div>
                              <div className="flex-1">
                                <div className="text-sm font-semibold text-white group-hover:text-blue-400 transition-colors flex items-center justify-between">
